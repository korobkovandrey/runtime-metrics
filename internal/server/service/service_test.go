...
func TestService_UpdateBatch(t *testing.T) {
	...
	t.Run("update batch", func(t *testing.T) {
		...
		mockRepository.EXPECT().FindBatch(gomock.Eq([]*model.MetricRequest{
		...
		mockRepository.EXPECT().CreateOrUpdateBatch(gomock.Eq(mrsReq)).Return(want, nil)
		service := NewService(mockRepository)
		got, err := service.UpdateBatch(mrs)
		assert.NoError(t, err)
